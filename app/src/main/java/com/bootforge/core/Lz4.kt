...
object Lz4 {
    ...
    fun decompressBlock(src: ByteArray): ByteArray {
        var out = ByteArray(max(1024, src.size * 2))
        var len = 0
        var i = 0
        val n = src.size
        ...
        while (i < n) {
            ...
            need(mlen)
            var p = len - offset
            for (k in 0 until mlen) out[len++] = out[p++]
        }
        return out.copyOf(len)
    }

    /**
     * Hash-table compressor (open addressing, no allocation per position).
     * Emits valid LZ4 block sequences: the last sequence is literal-only and
     * every match keeps the required 12 byte end-of-block margin.
     */
    fun compressBlock(src: ByteArray): ByteArray {
        val n = src.size
        val out = ByteArrayOutputStream(n / 2 + 64)
        ...
        while (i < limit) {
            ...
            out.write((min(litLen, 15) shl 4) or min(matchCode, 15))
            if (litLen >= 15) writeLength(out, litLen - 15)
            if (litLen > 0) out.write(src, anchor, litLen)
            val dist = i - cand
            out.write(dist and 0xFF)
            out.write((dist ushr 8) and 0xFF)
            if (matchCode >= 15) writeLength(out, matchCode - 15)
            i += mlen
            anchor = i
        }
        val litLen = n - anchor
        out.write(min(litLen, 15) shl 4)
        if (litLen >= 15) writeLength(out, litLen - 15)
        if (litLen > 0) out.write(src, anchor, litLen)
        return out.toByteArray()
    }

    private fun writeLength(out: ByteArrayOutputStream, value: Int) {
        var v = value
        while (v >= 255) {
            out.write(255)
            v -= 255
    ...
    fun compressLegacy(data: ByteArray): ByteArray {
        ...
        while (pos < data.size) {
            ...
            val block = compressBlock(data.copyOfRange(pos, pos + len))
    ...
    private fun writeLE32(out: ByteArrayOutputStream, v: Long) {
